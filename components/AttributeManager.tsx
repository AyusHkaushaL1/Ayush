...
// ===== DEDICATED COMPONENT FOR CATEGORIES =====
interface Category {
    id: string;
    title: string;
    slug?: string;
...
interface CategoryManagerProps {
    items: Category[];
    onAdd: (data: Omit<Category, 'id' | 'slug'>) => void;
    onEdit: (id: string, data: Omit<Category, 'id' | 'slug'>) => void;
    onDelete: (id: string) => void;
    isLoading: boolean;
}
const CategoryManager: FC<CategoryManagerProps> = ({ items, onAdd, onEdit, onDelete, isLoading }) => {
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [editingCategory, setEditingCategory] = useState<Category | null>(null);
    const [formData, setFormData] = useState({ title: '', image: '', size: '' });
    ...
    const handleOpenModal = (category: Category | null = null) => {
        ...
        if (category) {
            setFormData({
                ...
                size: category.size.join(', '),
            });
        } else {
            setFormData({ title: '', image: '', size: '' });
        }
        setIsModalOpen(true);
    };
    ...
    const handleSave = () => {
        ...
        if (editingCategory) {
            onEdit(editingCategory.id, processedData);
        } else {
            onAdd(processedData);
        }
        handleCloseModal();
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
            ...
            {isLoading ? (
                ...
                <div className="divide-y divide-gray-200">
                    {items.map(category => (
                        <div key={category.id} className="flex items-start justify-between py-3">
                            <div className="flex items-start space-x-4">
                                {category.image ? (
                                    <img src={category.image} alt={category.title} className="h-16 w-16 rounded-md object-cover" />
                                ) : (
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-gray-500 max-w-md">
                                        {category.size.length > 0 ? `Sizes: ${category.size.slice(0, 5).join(', ')}${category.size.length > 5 ? '...' : ''}` : 'No sizes specified'}
                                    </p>
                                </div>
                            </div>
                            <div className="flex items-center space-x-2 flex-shrink-0">
                                <button onClick={() => handleOpenModal(category)} className="text-blue-500 hover:text-blue-700 p-1 rounded"><Edit className="h-4 w-4" /></button>
                                <button onClick={() => onDelete(category.id)} className="text-red-500 hover:text-red-700 p-1 rounded"><Trash2 className="h-4 w-4" /></button>
                            </div>
                        </div>
                    ))}
                </div>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
                    <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-lg space-y-4">
                        <h4 className="text-xl font-bold">{editingCategory ? 'Edit' : 'Add'} Category</h4>
                        <div>
                        ...
                        <div>
                            ...
                            <input type="text" value={formData.image} onChange={e => setFormData({ ...formData, image: e.target.value })} className="mt-1 w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700">Sizes</label>
                            <textarea value={formData.size} onChange={e => setFormData({ ...formData, size: e.target.value })} className="mt-1 w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" rows={3} placeholder="Enter sizes, separated by commas"></textarea>
                        </div>
...
// ===== DEDICATED COMPONENT FOR COLLECTIONS =====
...
interface CollectionManagerProps {
    items: Collection[];
    onAdd: (data: Omit<Collection, 'id' | 'slug'>) => void;
    onEdit: (id: string, data: Omit<Collection, 'id' | 'slug'>) => void;
    onDelete: (id: string) => void;
    isLoading: boolean;
}

const CollectionManager: FC<CollectionManagerProps> = ({ items, onAdd, onEdit, onDelete, isLoading }) => {
    const [isModalOpen, setIsModalOpen] = useState(false);
    ...
    const handleSave = () => {
        ...
        if (editingCollection) {
            onEdit(editingCollection.id, processedData);
        } else {
            onAdd(processedData);
        }
        handleCloseModal();
    ...
    return (
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
            ...
            {isLoading ? (
                ...
                <div className="divide-y divide-gray-200">
                    {items.map(collection => (
                        <div key={collection.id} className="flex items-start justify-between py-3">
                            <div className="flex items-start space-x-4">
                                {collection.images[0] ? (
                                    <img src={collection.images[0]} alt={collection.title} className="h-16 w-16 rounded-md object-cover" />
                                ) : (
                                    <div className="h-16 w-16 rounded-md bg-gray-100 flex items-center justify-center">
                                        <ImageIcon className="h-8 w-8 text-gray-400" />
                                    </div>
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-gray-500 max-w-md">{collection.description || 'No description'}</p>
                                    {collection.isActive ? (
                            ...
                            <div className="flex items-center space-x-2 flex-shrink-0">
                                <button onClick={() => handleOpenModal(collection)} className="text-blue-500 hover:text-blue-700 p-1 rounded"><Edit className="h-4 w-4" /></button>
                                <button onClick={() => onDelete(collection.id)} className="text-red-500 hover:text-red-700 p-1 rounded"><Trash2 className="h-4 w-4" /></button>
                            </div>
                        </div>
                    ))}
                </div>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
                    <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-lg space-y-4">
                        <h4 className="text-xl font-bold">{editingCollection ? 'Edit' : 'Add'} Collection</h4>
                        <div>
                            <label className="block text-sm font-medium text-gray-700">Title</label>
                            <input type="text" value={formData.title} onChange={e => setFormData({ ...formData, title: e.target.value })} className="mt-1 w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" />
                        </div>
                        ...
                        <div className="flex items-center">
                            <input type="checkbox" id="isActiveCollection" checked={formData.isActive} onChange={e => setFormData({ ...formData, isActive: e.target.checked })} className="h-4 w-4 text-amber-600 border-gray-300 rounded" />
                            <label htmlFor="isActiveCollection" className="ml-2 block text-sm text-gray-900">Is Active</label>
                        </div>
                        <div className="flex justify-end space-x-3 pt-4">
                            <button onClick={handleCloseModal} className="px-4 py-2 bg-gray-200 rounded-lg text-sm">Cancel</button>
                            <button onClick={handleSave} className="px-4 py-2 bg-amber-600 text-white rounded-lg text-sm">Save</button>
...
const AttributeManager: FC = () => {
    ...
    const fetchData = useCallback(async (endpoint: string, stateSetter: (data: any[]) => void, options: { isPublic?: boolean } = {}) => {
        if (!options.isPublic && !authToken) return;
        setLoading(true);
        const headers: HeadersInit = {};
        if (!options.isPublic && authToken) {
        ...
        try {
            ...
            const formattedData = sourceArray.map((item: any) => {
                if (endpoint.includes('/productAttributes/categories')) {
                    return { id: item._id, title: item.title, slug: item.slug, image: item.image || '', size: item.size || [] };
                }
                if (endpoint.includes('settingStyles') || endpoint.includes('collections')) {
                    return { id: item._id, title: item.title, slug: item.slug, description: item.description, images: item.images || [], isActive: item.isActive };
                }
    ...
    const handleAdd = useCallback(async (endpoint: string, name: string, parentId?: string) => {
        ...
        if (endpoint.includes('colors') || endpoint.includes('shapes') || endpoint.includes('cuts') || endpoint.includes('clarities') || endpoint.includes('types') || endpoint.includes('subcategories')) { payload = { name: name.trim() }; }
    ...
    const handleEdit = useCallback(async (endpoint: string, id: string, newName: string, parentId?: string) => {
        ...
        if (endpoint.includes('colors') || endpoint.includes('shapes') || endpoint.includes('cuts') || endpoint.includes('clarities') || endpoint.includes('types') || endpoint.includes('subcategories')) { payload = { name: newName.trim() }; }
    ...
    const handleCategoryAdd = useCallback(async (data: Omit<Category, 'id' | 'slug'>) => {
        try {
            const response = await fetch(`${BASE_URL}/productAttributes/categories`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${authToken}` }, body: JSON.stringify(data) });
            if (!response.ok) throw new Error('Failed to add category');
            await fetchData('/productAttributes/categories', setCategories, { isPublic: true });
        } catch (error) { console.error('Error adding category:', error); }
    }, [authToken, fetchData]);

    const handleCategoryEdit = useCallback(async (id: string, data: Omit<Category, 'id' | 'slug'>) => {
        try {
            const response = await fetch(`${BASE_URL}/productAttributes/categories/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${authToken}` }, body: JSON.stringify(data) });
    ...
    const handleCollectionAdd = useCallback(async (data: Omit<Collection, 'id' | 'slug'>) => {
        try {
            const response = await fetch(`${BASE_URL}/productAttributes/collections`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${authToken}` }, body: JSON.stringify(data) });
            if (!response.ok) throw new Error('Failed to add collection');
            await fetchData('/productAttributes/collections', setCollections);
        } catch (error) { console.error('Error adding collection:', error); }
    }, [authToken, fetchData]);

    const handleCollectionEdit = useCallback(async (id: string, data: Omit<Collection, 'id' | 'slug'>) => {
        try {
            const response = await fetch(`${BASE_URL}/productAttributes/collections/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${authToken}` }, body: JSON.stringify(data) });
    ...
    const handleDelete = useCallback(async (endpoint: string, id: string) => {
        ...
        try {
            ...
            const isPublic = endpoint.includes('/productAttributes/categories');
            await fetchData(endpoint, getSetter(endpoint), { isPublic });
    ...
    const getSetter = (endpoint: string): React.Dispatch<any> => ({
        ...
        '/productAttributes/collections': setCollections,
    }[endpoint] || (() => {}));
    
    useEffect(() => {
        fetchData('/productAttributes/categories', setCategories, { isPublic: true });
        if (authToken) {
            ...
            fetchData('/productAttributes/collections', setCollections);
        }
    }, [authToken, fetchData]);
                ...
                return (
                    <div className="space-y-6">
                        <CategoryManager
                            ...
                            onDelete={(id) => handleDelete('/productAttributes/categories', id)}
                            isLoading={loading}
                        />
                        <AttributeList
                            title="Subcategories"
                            items={subcategories}
                            onAdd={(name, parentId) => handleAdd('/productAttributes/subcategories', name, parentId)}
                            onEdit={(id, newName, parentId) => handleEdit('/productAttributes/subcategories', id, newName, parentId)}
                            onDelete={(id) => handleDelete('/productAttributes/subcategories', id)}
                            isLoading={loading}
                            parentOptions={categories.map(c => ({ id: c.id, name: c.title }))}
                        />
                    </div>
                );
            case 'metal':
                ...
                return (
                    <div className="space-y-6">
                        <CollectionManager 
                            ...
                            onDelete={(id) => handleDelete('/productAttributes/collections', id)}
                            isLoading={loading}
                        />
