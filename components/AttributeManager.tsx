...
interface AttributeListProps {
  ...
  // CHANGE 1: onEdit prop signature updated to include optional parentId
...
const AttributeList: FC<AttributeListProps> = ({
  ...
  // CHANGE 2: editingItem state updated to include optional parentId
  ...
  // CHANGE 3: Item type is updated to include parentId
  ...
  const handleEditSave = () => {
    if (editingItem && editName.trim()) {
      // CHANGE 4: parentId from state is now passed to the onEdit function
...
const AttributeManager: FC = () => {
  ...
  const fetchData = useCallback(async (endpoint: string, stateSetter: (data: any[]) => void) => {
    ...
    try {
      ...
      // CHANGE 5: More robust mapping to safely extract parentId
  ...
  // CHANGE 6: handleEdit signature updated to accept optional parentId
  const handleEdit = useCallback(async (endpoint: string, id: string, newName: string, parentId?: string) => {
    ...
    // CHANGE 7: Add parentId to the payload if it exists (crucial for subcategories)
  ...
  const getSetter = (endpoint: string) => {
    const setters: { [key: string]: React.Dispatch<any> } = {
      'categories': setCategories, 'subcategories': setSubcategories,
      'metalAttributes/purities': setMetalQualities, 'metalAttributes/colors': setMetalColors,
      'diamondAttributes/shapes': setDiamondShapes, 'diamondAttributes/cuts': setDiamondCuts,
      'diamondAttributes/clarities': setDiamondClarities, 'diamondAttributes/colors': setDiamondTones,
      'settingStyles': setSettingStyles
    };
    return Object.keys(setters).find(key => endpoint.includes(key)) ? setters[Object.keys(setters).find(key => endpoint.includes(key))!] : () => {};
  };
      ...
      case 'categories':
        return (
          <div className="space-y-6">
            ...
            {/* CHANGE 8: The onEdit prop now correctly passes parentId to handleEdit */}
